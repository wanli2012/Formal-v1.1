...
@interface NIMSessionListViewController ()<NIMConversationManagerDelegate,NIMTeamManagerDelegate,LBGoodFriendsViewHeaderFooterViewdelegete,NIMUserManagerDelegate,NIMEventSubscribeManagerDelegate,NIMSystemNotificationManagerDelegate>

@property (strong, nonatomic) LBGoodFriendsHeaderView *goodFriendsHeaderView;
@property (assign, nonatomic) NSInteger  typeBtIndex;//点击的按钮类型 1为消息 2为通讯录 默认为消息
...
- (void)viewDidLoad {
    ...
    self.typeBtIndex = 1;
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 65, kSCREEN_WIDTH, kSCREEN_HEIGHT - 64) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.typeBtIndex == 1) {
        ...
        static NSString * cellId = @"ahah";
        UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cellId];
        if (!cell) {
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
           
        }
       cell.detailTextLabel.text = @"hahahh";
        return cell;
    }
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (self.typeBtIndex == 1) {
        sectionHeadView.goodFriendsSectionView.messageBt.selected = YES;
        sectionHeadView.goodFriendsSectionView.MailListBt.selected = NO;
        sectionHeadView.currentBt = sectionHeadView.goodFriendsSectionView.messageBt;
    }else{
        sectionHeadView.goodFriendsSectionView.messageBt.selected = NO;
        sectionHeadView.goodFriendsSectionView.MailListBt.selected = YES;
        sectionHeadView.currentBt = sectionHeadView.goodFriendsSectionView.MailListBt;
    }
    sectionHeadView.delegete = self;
    return sectionHeadView;
}

#pragma mark -- LBGoodFriendsViewHeaderFooterViewdelegete
//点击消息
