        ...
        case 1:{
            [[NIMSDK sharedSDK].teamManager quitTeam:self.team.teamId completion:^(NSError *error) {
                if (!error) {
                    ...
                    switch (error.code) {
                        case 801:
                            [self.view makeToast:[NSString stringWithFormat:@"群人数达到上限"]];
                            break;
                        case 802:
                            [self.view makeToast:[NSString stringWithFormat:@"没有权限"]];
                            break;
                        case 803:
                            [self.view makeToast:[NSString stringWithFormat:@"群已解散"]];
                            break;
                            
                        default:
                             [self.view makeToast:[NSString stringWithFormat:@"退出群组失败"]];
                            break;
                    }
                }
            }];
                ...
                if (!error) {
                    ...
                    switch (error.code) {
                        ...
                        case 803:
                            ...
                            break;
                        case 804:
                            [self.view makeToast:[NSString stringWithFormat:@"用户不在群"]];
                            break;
                       
                        default:
                             [self.view makeToast:[NSString stringWithFormat:@"解散群失败"]];
                            break;
                    }
