...
- (void)onRecvRevokeMessageNotification:(NIMRevokeMessageNotification *)notification
{
   ...
   DWTabBarController  *tabVC = [DWTabBarController instance];
        ...
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            switch ([dict jsonInteger:NTESNotifyID]) {
                case NTESCustom:{
                    ...
                    [[DWTabBarController instance].selectedViewController.view makeToast:content duration:2.0 position:CSToastPositionCenter];
                }
                    break;
...
{
    DWTabBarController *tab = [DWTabBarController instance];
    [tab.view endEditing:YES];
    if (tab.presentedViewController) {
        __weak DWTabBarController *wtabVC = tab;
...
{
    UINavigationController *nav = [DWTabBarController instance].selectedViewController;
    for (UIViewController *vc in nav.viewControllers)
    {
